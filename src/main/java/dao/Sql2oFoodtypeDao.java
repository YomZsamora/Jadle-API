...
public class Sql2oFoodtypeDao implements FoodtypeDao {
    ...
    public List<Restaurant> getAllRestaurantsForAFoodtype(int foodtypeId) {
        ...
        String joinQuery = "SELECT restaurantid FROM restaurants_foodtypes WHERE foodtypeid = :foodtypeId"; // Getting restaurantId when querying a certain FoodType

        try (Connection con = sql2o.open()) {
            List<Integer> allRestaurantIds = con.createQuery(joinQuery) // Once where get the restaurant's IDs, we add them to a list
